...
/*        PartInfo partInfo = parent.getEditingPartInfo();
    ...
    protected void actionPerformed(GuiButton button) {
        PartInfo orgPartInfo = parent.getEditingPartInfo();
        RenderPart part = PartRegistry.getRenderPart(parent.getPartType(), orgPartInfo.typeid);
        //Texture select
        if (button.id == 18) {
            if (parent.textureID - 1 >= 0) {
                ...
                parent.textureID = part.getTextureNames(orgPartInfo.subid).length - 1;
            }
        }
        else if (button.id == 19) {
            if (part.getTextureNames(orgPartInfo.subid).length > parent.textureID + 1) {
                parent.textureID++;
            }
            else {
                parent.textureID = 0;
        ...
        PartInfo partInfo = new PartInfo(true, orgPartInfo.typeid, orgPartInfo.subid, parent.textureID,
                orgPartInfo.tints, orgPartInfo.partType, null, orgPartInfo.pos, orgPartInfo.rot,
                orgPartInfo.mountPoint);
